...
export function PreparationJourney({ pkg, current }: { pkg: PackageListItem; current: Step }) {
  ...
  const steps = [
    { key: 'resume', label: NAMES.optimizedCv, ready: cvReady, href: `/package/${id}` },
    { key: 'letter', label: NAMES.coverLetter, ready: letterCount(pkg) > 0, href: `/cover-letter?package=${id}` },
    { key: 'qa', label: NAMES.interviewQa, ready: qaReady(pkg), href: `/interview-qa?package=${id}` },
    { key: 'mock', label: NAMES.mockInterview, ready: Boolean(reportRunId), href: `/mock-interview?package=${id}` },
  ...
  const note = current === 'letter' ? USES.coverLetter : current === 'qa' ? USES.interviewQa : current === 'mock' || current === 'report' ? USES.mockInterview : null
  return (
    <>
    {note ? <TargetJobCard pkg={pkg} note={note} className="mt-4" /> : null}
    <nav aria-label="Application preparation" className="mt-4 rounded-card border border-line bg-white p-4 shadow-m-1">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Link href={`/package/${id}`} className="inline-flex min-h-11 items-center text-sm font-semibold text-teal underline underline-offset-4">{CTA.viewOptimizedCv}</Link>
      </div>
      <ol className="grid grid-cols-2 gap-2 sm:grid-cols-5">
        {steps.map((step, index) => {
          const available = step.key === 'resume' || (cvReady && step.href !== null)
          const content = <><span className="block text-sm font-semibold">{index + 1}. {step.label}</span><span className="mt-1 block text-xs">{step.ready ? 'Saved' : step.key === 'report' ? 'After practice' : !available ? 'Optimize CV first' : 'To prepare'}</span></>
