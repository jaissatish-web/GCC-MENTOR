...
function bandColour(key: string): { ring: string; text: string; tint: string } {
  if (key === 'ready') return { ring: 'text-signal-ink', text: 'text-signal-ink', tint: 'bg-signal-tint' }
  if (key === 'mid') return { ring: 'text-signal-ink', text: 'text-signal-ink', tint: 'bg-gold-tint' }
...
function ScoreRing({ score, colour }: { score: number; colour: string }) {
  ...
  return (
    <svg viewBox="0 0 120 120" className="h-36 w-36 -rotate-90">
      <circle cx="60" cy="60" r={r} fill="none" strokeWidth="10" className="stroke-edge" />
      <circle
        cx="60"
        cy="60"
        r={r}
...
export function ScorecardResult({
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Hero */}
      <section className="rounded-bp-lg border border-edge bg-white p-6 sm:p-8">
        <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-center sm:gap-8">
          <div className="relative flex items-center justify-center">
            <ScoreRing score={result.finalScore} colour={colour.ring} />
            <div className="absolute flex flex-col items-center">
              <span className="font-mono text-4xl font-bold text-graphite">{result.finalScore}</span>
          ...
          <div className="flex flex-1 flex-col items-center gap-2 text-center sm:items-start sm:text-left">
            ...
            <h2 className="font-bp-display text-2xl text-graphite">{result.band.label}</h2>
            <p className="text-[14px] leading-relaxed text-graphite-soft">{result.band.message}</p>
          </div>
        </div>

        {result.lowResumeSignal ? (
          <p className="mt-5 rounded-bp border border-edge bg-paper/60 px-3.5 py-2.5 text-[12px] text-slate">
      ...
      {/* Dimension bars */}
      <section className="rounded-bp-lg border border-edge bg-white p-6 sm:p-8">
        <h3 className="text-[13px] font-bold uppercase tracking-wide text-slate">Your profile at a glance</h3>
              ...
              <div key={d.key} className="flex flex-col gap-1">
                <div className="flex items-center justify-between text-[13px]">
                  <span className="font-semibold text-graphite">
                    {d.label}
                    {d.confidence === 'low' ? (
                      <span className="ml-2 text-[12px] font-normal uppercase tracking-wide text-slate">low confidence</span>
                    ) : null}
                  </span>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-paper">
                  <div
                    className={`h-2 rounded-full ${pct >= 70 ? 'bg-signal' : pct >= 40 ? 'bg-signal' : 'bg-terra'} transition-all duration-700`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <section className="rounded-bp-lg border border-edge bg-white p-6">
          <h3 className="text-[13px] font-bold uppercase tracking-wide text-signal">Working in your favour</h3>
          <ul className="mt-3 flex flex-col gap-2.5">
            {result.strengths.length ? (
              result.strengths.map((s, i) => (
                <li key={i} className="flex gap-2 text-[13px] text-graphite-soft">
                  <span className="text-signal">✓</span>
                  {s}
                </li>
              ))
            ) : (
              <li className="text-[13px] text-slate">We could not detect clear strengths yet — the recommendations below are where to start.</li>
            )}
          </ul>
        </section>
        ...
        <section className="rounded-bp-lg border border-edge bg-white p-6">
          ...
          <ul className="mt-3 flex flex-col gap-2.5">
            {result.weaknesses.length ? (
              ...
              <li className="text-[13px] text-slate">No major gaps stood out — a tailored, optimised resume is your next step.</li>
            )}
          </ul>
        </section>
      </div>
      ...
      <section className="rounded-bp-lg border border-edge bg-white p-6 sm:p-8">
        <h3 className="text-[13px] font-bold uppercase tracking-wide text-slate">If you fix one thing, start here</h3>
        <div className="mt-4 flex flex-col gap-3">
          {(locked ? result.recommendations.slice(0, 1) : result.recommendations).map((r, i) => (
            <div key={i} className="rounded-bp-lg border border-edge bg-paper/50 p-4">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-mono text-[12px] text-slate">#{i + 1}</span>
                <span className="text-[14px] font-semibold text-graphite">{r.title}</span>
                <span className="rounded-full bg-white px-2 py-0.5 text-[12px] font-bold uppercase tracking-wide text-slate">
                  {r.impact} impact · {r.difficulty} effort
                </span>
              </div>
              <p className="mt-1.5 text-[13px] text-graphite-soft">{r.why}</p>
            </div>
          ))}
      ...
      {locked ? (
        <section className="rounded-bp-lg border border-signal/40 bg-gold-tint/40 p-6 text-center sm:p-8">
          <h3 className="font-bp-display text-2xl text-graphite">See your complete Gulf Readiness report</h3>
