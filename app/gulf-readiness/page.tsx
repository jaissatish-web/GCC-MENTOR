...
function ScoreCard({ label, value, tone }: { label: string; value: number; tone: 'green' | 'gold' | 'terra' }) {
  const ring = tone === 'green' ? 'border-teal bg-teal-soft' : tone === 'gold' ? 'border-teal bg-teal-soft' : 'border-alert bg-alert-soft'
  return <div className={`rounded-card border-2 ${ring} p-5`}><p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-500">{label}</p><div className="mt-2 flex items-end gap-1"><span className="font-mono text-4xl font-bold text-ink">{value}</span><span className="mb-1 text-sm text-ink-500">/100</span></div></div>
}
    ...
    <section className="rounded-card border border-line bg-white p-6">
      ...
      <ul className="mt-4 space-y-2.5">
        {signals.map((sig) => (
          <li
            key={sig.id}
            className={`flex gap-3 rounded-ctl border p-3 ${
              sig.present
                ? 'border-teal/30 bg-teal-soft/50'
          ...
          >
            <span
              ...
              className={`mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full text-[12px] font-bold ${
                sig.present ? 'bg-teal text-white' : 'bg-teal text-white'
              }`}
            >
              {sig.present ? '✓' : '!'}
            </span>
...
function NextStep({ score }: { score: Score }) {
  ...
  return (
    <section className="mx-auto mt-10 max-w-4xl overflow-hidden rounded-card bg-ink text-white shadow-redesign-cta-glow">
      <div className="p-8 sm:p-10">
        <p className="text-[12px] font-bold uppercase tracking-[0.2em] text-teal">
        ...
        {!ready && wins.length > 0 ? (
          <div className="mx-auto mt-6 max-w-xl rounded-card border border-white/15 bg-white/[0.06] p-5 text-left">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-[12px] font-semibold uppercase tracking-wider text-teal">
                Your quickest wins
              </span>
              <span className="font-mono text-sm text-white/70">
                {score.overall_score} → <span className="font-bold text-teal">{potential}</span>
              </span>
            </div>
            <ul className="mt-3 space-y-2">
              {wins.map((w) => (
                <li key={w.id} className="flex items-center justify-between gap-4 text-sm">
                  ...
                  <span className="shrink-0 font-mono text-[13px] font-bold text-teal">
                    +{w.impact}
                  </span>
                </li>
              ))}
...
export default function GulfReadinessPage() {
  ...
  if (error) return <main className="min-h-dvh bg-canvas px-5 py-20 text-center"><h1 className="font-display text-4xl">Your scan is unavailable</h1><p className="mx-auto mt-4 max-w-lg text-ink-soft">{error}</p><Link href="/ats-scan" className="mt-8 inline-flex rounded-ctl bg-teal px-6 py-3 font-bold text-white">Scan my CV again</Link></main>
  ...
  return <main className="min-h-dvh bg-canvas text-ink"><header className="border-b border-line bg-canvas/95"><div className="mx-auto flex h-[72px] max-w-[1100px] items-center justify-between px-5 sm:px-8"><Link href="/" className="flex items-center gap-2.5"><span className="font-display flex h-9 w-9 items-center justify-center rounded-ctl bg-teal text-lg text-white">G</span><span className="font-bold tracking-wide">GCC MENTOR</span></Link><Link href="/login" className="text-sm font-semibold text-ink-muted">Log in</Link></div></header>
    <div className="mx-auto max-w-[1100px] px-5 py-12 sm:px-8 lg:py-16"><div className="mx-auto max-w-4xl text-center"><p className="text-[12px] font-bold uppercase tracking-[0.2em] text-teal">Your free CV scan</p><h1 className="mt-4 font-display text-5xl leading-tight sm:text-6xl">Here is what is holding your CV back.</h1><p className="mx-auto mt-5 max-w-3xl text-lg leading-relaxed text-ink-soft">{score.summary}</p></div>
