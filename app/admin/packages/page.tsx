    ...
    >
      ...
      <Card className="flex flex-col gap-4 p-5">
        ...
        <p className="text-[12px] text-slate">
          ...
          <code className="mx-1 rounded bg-paper px-1.5 py-0.5 font-mono text-[12px]">resume_optimization</code>
          and
          <code className="mx-1 rounded bg-paper px-1.5 py-0.5 font-mono text-[12px]">cover_letter</code>.
        ...
        <form action={createServicePackageAction} className="flex flex-col gap-3" id="create-service-package-form">
          ...
          <div className="flex flex-col gap-2">
            <div className="text-[12px] font-semibold uppercase tracking-wide text-slate">Quota line items</div>
            <p className="text-[12px] text-slate">The service package is only available for purchase from here.</p>
        ...
        {servicePackages.length > 0 ? (
          <div className="flex flex-col gap-2">
            ...
            {servicePackages.map((p) => (
              <div key={p.id} className="flex flex-col gap-2 rounded-bp-lg border border-edge p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-semibold text-graphite">{p.name}</span>
                  ...
                  <div className="flex items-center gap-2">
                    {p.isActive ? (
                      ...
                      <form action={setServicePackageActiveAction}>
                        ...
                        <button type="submit" className="text-[12px] font-semibold text-signal underline-offset-2 hover:underline">Activate</button>
                      </form>
                    )}
                  </div>
                </div>
                <div className="flex flex-wrap gap-1">
                  {p.items.map((item) => (
                    <span key={item.serviceKey} className="rounded-md border border-edge bg-paper px-2 py-0.5 text-[12px] font-mono text-graphite-soft">
          ...
          <p className="text-[12px] text-slate">No service packages created yet.</p>
        )}
      </Card>
    </PageShell>
  )
