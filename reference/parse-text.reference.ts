...
3. For gulf_fields: actively look for visa status, iqama, NOC, driving licence, salary expectations, accommodation preferences, notice period. These may appear anywhere in the resume.
4. For experience.highlights: preserve each bullet point as a separate string in the array. Keep the original wording.
5. For skills: categorize intelligently into technical (tools/systems), software (specific product names), standards (codes/standards like IEC/ISA/API), and soft (interpersonal skills).
6. For dates: use YYYY-MM format where possible. If only year is available, use YYYY. If current job, use "present" for end_date.
7. Generate a unique ID string for each item that has an "id" field (use format like "exp-1", "edu-1", "cert-1", "proj-1").
8. Set order field as sequential integers starting from 1.
9. If the resume mentions a country in Gulf region (Saudi Arabia, UAE, Qatar, Kuwait, Oman, Bahrain), add it to gulf_fields.target_countries.
...
REQUIRED OUTPUT SCHEMA:
{
  ...
  "gulf_fields": {
    ...
    "iqama_transferable": "boolean or null",
    "notice_period": "string or null",
    "gcc_driving_licence": "string or null",
    "current_location": "string or null",
    "noc_available": "boolean or null",
    "expected_salary": "string or null",
    "accommodation_pref": "string or null",
    "target_countries": []
  },
  "summary": "string",
