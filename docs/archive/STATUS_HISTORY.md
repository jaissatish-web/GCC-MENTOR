...
**What that batch actually contains** — the two that matter most: **TASK-107** fixed PDF extraction fabricating resume facts. Subset-font encodings made extraction emit a consistent-looking cipher with every digit silently deleted; 19k characters of noise cleared the length check, reached the model, and the model invented specifics — it reported "18,000+ loop checks with 99.8% accuracy" where the founder's resume says 50,000+ and 99.2%. A direct breach of the product's one promise, on a real CV. **TASK-108** made the free GCC readiness report fully deterministic: 97s → 0.6s, zero cost, and repeatable, replacing a model scorer that returned 78 and then 45 for the same resume. The rest: the navy+light theme conversion (TASK-112) and the three rounds of contrast/CTA fixes it took to land (114/115/116/119), private profile-photo storage (TASK-113, migration 032 applied), mobile overflow (TASK-117), and the Career Profile form redesign (105/106/119/120/121).

**Two standing blockers closed by that batch.** Unplanned #20 (magic-link sign-in) is fixed — it was a real production bug, not just a verification gap: Supabase authenticated users correctly and the app told them sign-in had failed. Unplanned #19 (PDF font encoding) is fixed by TASK-107.

**The one defect found in review — TASK-122, fixed and verified.** TASK-108 correctly made extraction conditional on a job description, but session persistence was still gated on the extracted draft, so **for every scan without a JD — the default path — nothing was stored at all**. Refreshing the results page said "Your scan is unavailable", the page's own "kept for 7 days" promise was false, and signup had nothing to claim. Persistence is now gated on the resume text; signup pre-fill is restored by extracting at claim time behind the existing progress screen, so the fast scan and the "never upload twice" promise both hold. Three smaller honesty fixes went in with it: dead imports left behind when the route stopped calling the model, an admin prompt-template editor that no longer feeds any AI call (now labelled instead of silently doing nothing), and a `/gulf-readiness` line promising score tracking that does not exist. Verified by round-tripping the real path against a running server and reloading the page in a browser, not by reading the diff.

**Also independently re-verified this session:** migration 032 is genuinely applied — bucket `profile-photos` exists with `public=false`, and an unauthenticated probe confirmed writes denied by RLS, signed-URL minting denied, public URL not served.

**THE AI PIPELINE IS LIVE — the longest-standing blocker in this file is closed, and was closed before today.** The founder set an OpenRouter key in `/admin` on **2026-08-13**; `ai_provider_config` has held a real `default` row (openrouter · `deepseek/deepseek-v4-flash`) since then. Every earlier entry in this file saying "the key has never been set", including the first version of this one, was stale — it was written from the previous status entry rather than checked. Confirmed live 2026-08-16 by running the real funnel: a free scan with a job description returned a genuine Job Match report (three real model calls — JD structuring, deterministic mapping, LLM explanation), and the account shows real usage. **Nothing about AI is blocked any more.**

**Two things that surfaced from finally exercising it:**

1. **Job Match scores are confidently wrong for a strong candidate, and this is the product's differentiator.** A test CV with 12 years of oil-and-gas experience in Abu Dhabi and Jubail, against a matching Senior Piping Engineer JD, scored **48/100** — with `gcc_experience: 0`, `experience_level: 0` and `education: 0`. The GCC zero has a definite cause: `gccExperienceCategory()` in `lib/jobMatch/requirementMapping.ts` counts only work entries carrying a `gcc_country` value, which is a Career Profile column (TASK-067) that **extraction never populates**, so for every anonymous scan that category is structurally always 0 no matter what the CV says. Education 0 came from `B.Tech` not matching a JD asking for `B.Eng`. The semantic layer scored the same candidate 85/95/100 and its own diagnosis says the screening "gave zeros" — so the deterministic half is dragging an honest 90 down to 48. **Needs its own ticket; it is a scoring-semantics decision, not a typo.**
2. **Provider errors were undiagnosable.** The first live run failed with "Primary AI provider failed and no valid fallback is configured" — the real cause was discarded (passed as `cause`, which nothing logs). Fixed: the underlying provider message is now carried into the thrown message, and an empty-content response now reports `finish_reason` and whether the model returned reasoning only — the actual failure mode for reasoning models like the configured default, whose thinking tokens are billed against the same `max_tokens` budget before any visible content. Note the first run failed and an identical second run succeeded, so there is some intermittency here worth watching.

**What is still open:** the claim-on-signup path and the new-user dashboard branch both need a real new account and remain unverified.)

**Last updated:** 2026-08-14, end of day (**TASK-103 — dashboard navigation + cross-page consistency pass, founder-specified.** Presentation/navigation only; no backend, API, query or AI behaviour touched. Sidebar reordered and renamed to the requested 9 items and, more usefully, **de-duplicated**: the list existed three times (sidebar, mobile bar, More drawer) and had already drifted — the mobile bar still said "Library" and still showed Payments — so all three now render from one typed array in `components/layout/navItems.ts`. New `/create-resume` page offers Upload / Paste / Type, each routing into the extraction pipeline that already existed rather than duplicating it. Settings restructured into Account · Email · Current Package · Payments · Delete Data using real data only (service credits, actually-paid packages) — no invented subscription tier. **Three genuine defects fixed in passing, all pre-existing:** (1) `/payments` was missing from `middleware.ts` entirely, so it was an unauthenticated route — now protected, along with `/create-resume`, in *both* the route list and the matcher; (2) the Resume Optimizer felt like a separate product because it literally was — `/optimize/target` and `/optimize/setup` were light-themed with **no AppShell and therefore no sidebar at all**; both now wrapped and converted to dark tokens, and their `ring-forest-deep` focus rings (near-invisible on dark) moved to gold; (3) the founder's contrast complaint was measurable — helper text app-wide used opacity washes (`text-ink-900-dark/40`–`/60`) instead of the `ink-400-dark` token that exists for exactly that, 22 instances in `/profile` alone, all converted. Career Profile gained per-section guided helper text, an Optional marker and a section jump-nav (deliberately not collapsible sections — that hides fields users still must fill and buries validation errors). `tsc`/`lint`/full `build` clean; all nine nav destinations verified to resolve (307→login, vs 404 for a control route, which is what proves they exist). **New Unplanned #20:** magic-link sign-in cannot complete — `app/auth/callback/route.ts` only handles the PKCE `?code=` flow, but Supabase returned implicit-flow fragment tokens; Supabase authenticated fine, the app-side callback rejected it. Pre-existing and untouched, but it is the reason no authenticated page has ever been visually verified in this environment — and it may mean emailed magic-link login is broken in production, depending on the project's configured auth flow. Worth checking.)

**Last updated:** 2026-08-14, later same day again (**TASK-102 reviewed, ticketed and pushed — a real user-facing 500 on profile save.** Founder asked for a push; found unticketed work already sitting uncommitted in the working tree (the same recurring process gap as Unplanned #15 — work landing without a ticket). Reviewed the full diff before committing rather than pushing it unread. **It's genuinely good and fixes a real bug**: resumes give dates to month precision at best ("March 2021"), extraction correctly returns `YYYY-MM` rather than inventing a day, but the Postgres `date` columns and the form's `<input type="date">` both reject that — the input silently blanked the value while it stayed in React state, and saving 500'd with no visible cause, on the main "confirm your extracted profile" screen. Fixed with a new `lib/partialDates.ts` (pads for storage, never shows the padded day, returns null rather than guessing on unparseable input) plus `type="month"` inputs for work/certification dates. A second independent bug fixed in the same diff: `readinessInput` aliased `profileRow` instead of copying it, so the child arrays leaked non-column keys onto the upsert and failed it. Verified `tsc`/`lint`/full `build` all clean; **not** exercised against a live authenticated save (standing environment gap). One tradeoff recorded in the ticket: existing rows that did have a real day will narrow to the 1st on next save. **Also**: eight untracked `tmp-*.mjs` debug scripts were sitting in the repo root — they read `.env.local` at runtime (several use the service-role key) rather than hardcoding secrets, so nothing sensitive was about to be committed, but `.gitignore` now excludes them and `.hermes/`.)

**Last updated:** 2026-08-14, later same day (**TASK-101 done — landing page "wow factor" redesign, founder-directed.** Founder saw the deployed page on Vercel, called it generic, and asked for real Gulf/oil-and-gas imagery, a hard-hat engineer photo, a graph, and copy establishing the platform was built by a genuinely experienced engineer. Flagged the "15+ year Gulf engineer" claim back to the founder before writing anything, per the standing never-invent rule; founder supplied his own resume PDF, confirming it's real. Built: a new founder-credibility section citing only verified facts (NEOM Green Hydrogen Complex, ADNOC TAKREER, Bechtel Al Taweelah, NSRP Vietnam, zero LTI, Saudi Aramco/ADNOC/Bechtel/Shell/QatarEnergy standards) with no invented numbers (digits were unrecoverable from the resume PDF — see the new Unplanned #19 finding below); a before/after resume-transformation panel plus an illustrative SVG readiness-score chart, both labeled illustrative per the existing honesty convention; three real, individually-verified Unsplash photos (hero oil/gas plant backdrop, hard-hat engineer in the credibility section — captioned neutrally, not implied to be the founder — and a second plant photo in the trust section). **Also fixed in passing**: `app/page.tsx` was using numbered-scale color classes (`forest-100/700/900`, `gold-500`, etc.) that don't exist in `tailwind.config.ts` — silently dropped by Tailwind rather than erroring, so the page rendered but without its intended accent colors on many elements. Replaced every occurrence with the real token names. `tsc`/`lint`/`build` all clean, live-checked on a dev server (zero console errors, all three images confirmed loading through Next's image optimizer). Full writeup in `docs/TASKS.md` TASK-101. **New Unplanned finding (#19)**: `lib/pdfTextExtract.ts`, the same extractor real users' resumes go through, cannot handle custom-encoded/subsetted PDF fonts — confirmed via the founder's own resume, which needed a constant-shift hand-decode. Digits don't decode at all under such encodings, meaning numeric resume content can silently vanish for real users. Not fixed here (out of scope for this ticket) — flagged for its own ticket.)

**Last updated:** 2026-08-14 (**Found and partially fixed a batch of unreviewed work — 20 commits, 18 from Hermes + 2 from the founder directly, landed and were pushed to GitHub between 2026-08-13 16:32 and 2026-08-14 08:41 with zero record in this file.** This is the same class of gap as the TASK-100 entry below (landing-page work built and pushed outside the ticket/review loop) — recurring, not a one-off. Full CTO review pass, `docs/TASKS.md`'s Unplanned findings #15 has the complete detail; summary here:
- **Fixed — landing page pricing (`app/page.tsx`)**: the ₹999/₹2,499 tiers had "Get Started" buttons leading to the same ₹499-only checkout as every other tier — no real self-serve purchase exists for the bundles. Added a `live` flag, moved "Most popular" to the real ₹499 tier, non-live tiers now show "Coming soon" instead of a working-looking button, restored an honest disclosure line. This was public-facing and could have caused a real customer to pay expecting more than they'd receive.
- **Fixed — AI Provider page (`app/admin/ai-provider/page.tsx`, `app/admin/actions.ts`)**: kept the founder's own genuinely good addition (a real cross-provider fallback, tried only when the primary fails) but restyled off a dark-navy/amber/violet gradient look back onto the flat forest/gold system every other admin page uses, restored the "Other overrides" management UI that had silently disappeared (backend still fully supported it), replaced `config: any` with the real type, and removed a provider dropdown option ("Other") confirmed to save successfully then throw `Unsupported AI provider: other` on the next real AI call — not hypothetical, traced directly through `lib/ai/provider.ts`.
- **Migration 031 applied and verified**: the fallback feature referenced two DB columns with no migration anywhere in the repo. Wrote `supabase/migrations/031_ai_provider_fallback_columns.sql` (idempotent, `IF NOT EXISTS`), founder supplied the pooler connection string, ran it inside a transaction, and independently confirmed against live `information_schema.columns` — not just "no error" — that `fallback_provider` and `fallback_api_key` both genuinely exist now. The AI Provider page's fallback feature is confirmed working end-to-end at the schema level.
- **Flagged, not changed**: a new page `/gulf-readiness` (ATS scan results, moved off `/ats-scan`) is well-built but confusingly named one letter off from the existing `/gcc-readiness`. The hand-rolled PDF extractor (`lib/pdfTextExtract.ts`) looks structurally sound (no eval, no ReDoS-prone patterns, size-capped) but wasn't live-tested. The anonymous scan rate limit raised 3→20/day is a reasonable, low-risk call, left alone.
`tsc`/`lint`/`build` clean after every fix.)
