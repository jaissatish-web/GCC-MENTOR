...
- [ ] **TASK-038: Rate limiting** — `lib/rateLimit.ts` enforced **server-side in the route, before the model call**. Default 5 extraction attempts/day via env var. Secondary keying on phone/email. Clear reset-time message on limit hit, never a silent failure. Depends on: TASK-010 · Status: done
- [ ] **TASK-039: AI usage logging** — write to `ai_usage_log` on every provider call, inside `lib/ai/provider.ts` so no route can forget. Depends on: TASK-010, TASK-015 · Status: done
