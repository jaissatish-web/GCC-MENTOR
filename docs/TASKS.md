...
- [ ] **TASK-038: Rate limiting** — `lib/rateLimit.ts` enforced **server-side in the route, before the model call**. Default 5 extraction attempts/day via env var. Secondary keying on phone/email. Clear reset-time message on limit hit, never a silent failure. Depends on: TASK-010 · Status: **CHANGES REQUESTED, not done.** CTO review found a real race condition: `incrementRateLimit` does read-count-then-upsert-count+1 as two separate operations, non-atomic. Two concurrent requests from the same user can both read the same count and both write the same incremented value, losing an increment — the exact scenario (rapid repeat calls) most likely under real abuse. Fix: a single atomic upsert, e.g. `upsert(..., { count: sql\`rate_limits.count + 1\` })` or an `ON CONFLICT DO UPDATE SET count = rate_limits.count + 1` RPC — not read-then-write. Everything else in this ticket is approved as-is: secondary keying via `career_profiles.{phone,email}` confirmed correct (the only canonical source in this project's data model), fail-closed on DB error confirmed correct, identity-lookup fail-open-to-own-id-only confirmed an acceptable degraded state. Re-submit after the atomic-increment fix only.
