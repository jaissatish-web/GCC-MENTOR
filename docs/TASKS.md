...
- [ ] **TASK-038: Rate limiting** — `lib/rateLimit.ts` enforced **server-side in the route, before the model call**. Default 5 extraction attempts/day via env var. Secondary keying on phone/email. Clear reset-time message on limit hit, never a silent failure. Depends on: TASK-010 · Status: **CHANGES REQUESTED (round 2), not done.** Round 1's atomic-increment fix (`supabase/migrations/016_increment_rate_limit.sql`, commit `f7c4b2e`) correctly solves the race — `ON CONFLICT ... DO UPDATE SET count = rate_limits.count + 1` in one statement is the right pattern, approved as written. But the migration creates a `SECURITY DEFINER` function with no `REVOKE`/`GRANT` at all. Postgres grants `EXECUTE` on new functions to `PUBLIC` by default (opt-out, unlike tables), and Supabase's PostgREST layer auto-exposes any function a caller's role can execute as `/rpc/increment_rate_limit`. Combined with `SECURITY DEFINER` (runs with the owner's privileges, bypasses RLS regardless of caller), this means **any authenticated — possibly anonymous — user can currently call this RPC directly from the browser with an arbitrary `p_user_id`**, no ownership check in the function body, and manipulate or exhaust any other user's rate-limit counters. Worse than the bug it replaced: the original race leaked at most a couple of free attempts; this is a working IDOR. Fix: add `REVOKE EXECUTE ON FUNCTION public.increment_rate_limit(uuid,text,date) FROM PUBLIC;` then `GRANT EXECUTE ... TO service_role;` only — same pattern already used twice in this project (`pii_access_log` migration 013, `ai_usage_log` migration 015). Everything else — the atomic-upsert logic itself, secondary keying, fail-closed/fail-open behavior — stays approved, untouched, do not rebuild it.
