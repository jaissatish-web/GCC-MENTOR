...
- [ ] **TASK-020: Rewrite extraction to the Career Profile schema**
      ...
      Depends on: TASK-015, TASK-011, TASK-038, TASK-039 · Status: not started — dependency corrected by CTO: the spec text requires the TASK-038 rate limiter and TASK-039's usage logging, but neither was listed. Build 038+039 first. **Contract resolved by CTO** after Hermes correctly stopped rather than guess (see Unplanned #11): "shape the returned draft to match `types/careerProfile.ts`" now means the new exported `CareerProfileDraft` type in that file — not `CareerProfileFull`. Two routes to create: `app/api/parse/upload/route.ts` and `app/api/parse/text/route.ts` (neither exists yet; reuse the parsing logic in `reference/parse-{upload,text}.reference.ts`, never its prompts, per `docs/HERMES.md` §7). Extraction must NOT populate `currently_in_gulf`, `current_employer`, `current_project`, `target_job_title`, `target_industry`, `target_country`, or `target_company` — status and target are the user's current situation and forward intent, not resume facts, and leaving them out keeps all three onboarding paths consistent (manual entry already collects them with zero AI help). Every other field in `CareerProfileDraft` is optional — a field extraction can't find is absent, never a fabricated placeholder. Child items get `sort_order` as their extraction-order index (a real fact, not an invention); no `id`/`profile_id`/`created_at` — those don't exist pre-save.
