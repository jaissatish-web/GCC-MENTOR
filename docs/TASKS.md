...
- [ ] **TASK-020: Rewrite extraction to the Career Profile schema**
      ...
      Depends on: TASK-015, TASK-011, TASK-038, TASK-039 · Status: **CHANGES REQUESTED, not done.** Contract resolved by CTO after Hermes correctly stopped rather than guess (see Unplanned #11) — `CareerProfileDraft`, not `CareerProfileFull`, status/target fields entirely absent. `normalizeDraft` verified by the CTO with 29 executed adversarial test cases (a simulated model actively trying to inject `id`, `target_job_title`, `readiness_score`, etc.) — every forbidden field dropped structurally, not just prompt-instructed. The extraction system prompt's own anti-hallucination rules (never invent, omit rather than fabricate) were Hermes's own addition beyond the ticket's literal ask — good instinct, keep it. Three required fixes before re-submitting: (1) `app/api/parse/text/route.ts` has no maximum length on pasted text, only a minimum — matches the reference file's own pre-existing gap, but an unbounded paste costs one rate-limit slot regardless of size, directly against this ticket's stated cost-control purpose; add an upper bound (~20,000 characters), 400 above it. (2) Rate-limit rejection returns `403`; change to `429` in both routes — the HTTP status purpose-built for rate limiting, not a style preference. (3) `maxTokens: 4096` risks silent truncation → malformed-JSON 422 for a senior candidate with many entries; raise to `8192` in both routes — a few rupees against a ₹30 budget, cheaper than a failed extraction. Not required to fix: `incrementRateLimit` firing only on success (not on 422/502) is confirmed as the correct UX call — logged as Unplanned #12, not a defect.
