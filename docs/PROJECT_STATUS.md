...
**What is still open, unchanged:** the OpenRouter key has still never been set in `/admin`, so every AI-dependent feature — optimization, cover letter, job match, extraction — has never produced a real result. Note that the *free scan no longer needs it*, which is why it could be tested end-to-end today. The claim-on-signup path and the new-user dashboard branch both need a real new account and remain unverified.)
