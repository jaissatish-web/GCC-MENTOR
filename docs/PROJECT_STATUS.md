...
- **Written, not yet applied — migration 031**: the fallback feature references two DB columns with no migration anywhere in the repo. Wrote `supabase/migrations/031_ai_provider_fallback_columns.sql` (idempotent, `IF NOT EXISTS`, safe whether or not the founder already added them by hand). **Needs the founder to run it or confirm the columns already exist** — no DB route from this sandbox to check directly.
