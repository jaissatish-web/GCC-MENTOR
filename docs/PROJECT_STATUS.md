...
**TASK-061 (admin UI for packages) — round 1 sent back, not approved.** `tsc`/`lint`/`build` all passed and the report claimed "repeatable service_key/quota rows," but the actual rendered form only has ONE hard-coded row — no way to add a second service through the UI, so an admin currently cannot build the ticket's own example bundle ("3 optimizations + 2 cover letters"). Caught by reading the actual JSX, not trusting the passing build. Round 2 spec is written: extract the quota-items list into a small client component with add/remove, following the same repeatable-row pattern already used in `/profile`'s work-experience editor. Everything else from round 1 (list view, toggle, server actions) stays as built. Next after this: per-call model selection, then versioned prompts, then Cover Letter itself — none of those three are built yet. OCR/scanned-document support was explicitly dropped from scope, founder's call. See `docs/TASKS.md` TASK-049/058/059/060/061 for full writeups.)
