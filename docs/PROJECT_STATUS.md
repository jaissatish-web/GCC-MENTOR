...
You can now build real multi-service packages from `/admin`. Next after this: per-call model selection, then versioned prompts, then Cover Letter itself — none of those three are built yet. OCR/scanned-document support was explicitly dropped from scope, founder's call. See `docs/TASKS.md` TASK-049/058/059/060/061 for full writeups.)
