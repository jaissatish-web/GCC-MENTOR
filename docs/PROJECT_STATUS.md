...
**New: prompt wording is now admin-editable too, matching how AI provider/model/key already were.** Founder wants full control from `/admin` — provider/model/key/fallback was already there; prompt *wording* wasn't. CTO built the underlying mechanism directly (migration 025, `prompt_templates` table; `lib/ai/promptTemplates.ts`) — deliberately scoped to ONE editable slot so far, the ATS scanner's intro/tone paragraph, with the safety-critical grounding rule and output JSON schema staying hard-coded and never overridable, by design (an admin-editable grounding rule would be able to weaken the product's core no-invention promise). Migration 025 is already applied and verified. **TASK-059 (the admin UI for it) is written and ready for Hermes** — pure CRUD form, no prompt-logic changes. See `docs/TASKS.md` TASK-049/058/059 for the full writeups.)
